...
| `SHORTSENGINE_YOUTUBE_INGEST_TIMEOUT_MS` | No | `120000` | integer `1000..900000` | No | Backward-compatible downloader timeout. Keep bounded; raise only for known long videos within upload limits. | Timeout returns `YOUTUBE_DOWNLOAD_TIMEOUT` with progress/stall diagnostics. |
| `SHORTSENGINE_YOUTUBE_DOWNLOAD_TIMEOUT_MS` | No | empty | integer `1000..900000` | No | Preferred downloader timeout alias for operator proof; overrides `SHORTSENGINE_YOUTUBE_INGEST_TIMEOUT_MS` when set. | Invalid values fail config/env validation before downloader work. |
...
| `SHORTSENGINE_YOUTUBE_FORMAT_SELECTOR` | No | `bv*[ext=mp4]+ba[ext=m4a]/b[ext=mp4]/best` | bounded yt-dlp format selector using safe characters only, including optional height filters such as `b[height<=720][ext=mp4]/best[height<=720]/best` | No | Prefer MP4-compatible video/audio so FFprobe and render validation stay predictable. | Invalid selectors fail env/config validation before downloader work. |
...
| `SHORTSENGINE_YOUTUBE_SMOKE_REQUEST_TIMEOUT_MS` | No | computed from downloader timeout `* SHORTSENGINE_YOUTUBE_DOWNLOAD_ATTEMPTS + 30000`, minimum `120000` | integer `1000..1800000` | No | Override only for authorized videos whose ingest request needs more time while the downloader stages media; default covers the bounded retry cycle. | Invalid timeout fails smoke; request timeout returns structured ingest diagnostics instead of misleading success. |
...
| `SHORTSENGINE_YOUTUBE_LIVE_E2E_TIMEOUT_MS` | No | `1800000` | integer `1000..1800000` | No | Keep bounded; lower for short local proof runs if needed. | Invalid timeout fails readiness/proof. |
| `SHORTSENGINE_YOUTUBE_LIVE_E2E_DOWNLOAD_TIMEOUT_MS` | No | empty | integer `1000..900000` | No | Operator-only long-source override; passed to the spawned proof server as the real downloader timeout after rights/URL gates pass. | Invalid values fail readiness/proof before server work. |
...
| `SHORTSENGINE_YOUTUBE_LIVE_E2E_DOWNLOAD_TIMEOUT_MS` | Optional bounded downloader timeout for authorized long-source proof. It does not enable ingest by itself and never bypasses rights/allowlist gates. |
...
`npm run env:check` validates these live proof flags too. The user must explicitly confirm usage rights before validation or ingest. Playlists, live streams, credentialed URLs, unsupported hosts, embeds, channels and search pages are rejected before any downloader call. Downloader failures are classified into safe codes such as `YOUTUBE_AUTH_REQUIRED`, `YOUTUBE_BOT_CHECK_REQUIRED`, `YOUTUBE_COOKIES_REQUIRED`, `YOUTUBE_VIDEO_PRIVATE`, `YOUTUBE_VIDEO_UNAVAILABLE`, `YOUTUBE_GEO_RESTRICTED`, `YOUTUBE_AGE_RESTRICTED` and `YOUTUBE_RATE_LIMITED`; timeout diagnostics also distinguish `DOWNLOAD_TIMED_OUT_WITH_PROGRESS` from `DOWNLOAD_STALLED_NO_PROGRESS`. The recovery path is another public video, retry when marked retryable, a bounded longer operator timeout for rights-cleared long sources, or MP4 fallback until authorized import is built. Public responses, doctor output and smoke reports never include local paths, storage keys, raw stdout/stderr, signed tokens or secrets.
...
When `SHORTSENGINE_YOUTUBE_LIVE_E2E_SCOREBOARD_OCR=1` is enabled, the live proof sets a bounded 300000ms smoke job timeout unless the operator explicitly provides `SHORTSENGINE_YOUTUBE_SMOKE_JOB_TIMEOUT_MS`. This keeps long scorebug OCR observable without changing the default smoke timeout.

Run `npm run proof:readiness` before asking ShortsEngine for a fresh MP4 proof. It is a no-network, no-downloader, no-server source readiness check. With no configured source it writes a skipped safe report to `demo/results/proof-source-readiness-latest.json`. With a local MP4 source it validates rights confirmation, MP4 signature, expected counted-goal count and safe public source metadata. With a live YouTube proof source it validates the opt-in flags, rights confirmation, URL allowlist/manual gate, expected counted-goal count and `youtube:doctor` runtime readiness. It never starts ingest, never reuses an old MP4 as success, and reports only safe relative refs.
...
Long-source scorebug OCR reports keep actual sampled frame counts separate from planned attempt diagnostics. Chunk summaries expose `plannedFrameCount`, `attemptedRoiCount`, and `attemptedObservationCount` whenever sampled timestamps and candidate ROI ids exist, even if the scorebug is unreadable. These fields are debugging evidence only: they explain which scorebug regions were attempted and why they failed, but they do not create score changes or confirm goals.
