...
- Treat credential-shaped object keys as sensitive in reports and logs, including `clientSecret`, `refreshToken`, `privateKey`, `accessKeyId`, `sessionToken`, authorization headers and raw stdout/stderr fields. Allow only explicit readiness booleans such as `credentialsConfigured`, `tokensRequested`, `secretsIncluded`, `logsDownloaded` and `artifactsDownloaded`.
...
- Multi-moment edit plans must validate segment count, source bounds, total duration, chronology, overlap, intro exclusion, reaction-as-support usage and no-false-goal captions before FFmpeg rendering.
