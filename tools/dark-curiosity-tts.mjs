...
const valueFlags = new Set(["--fixture", "--project", "--provider", "--voice", "--model", "--speaking-rate", "--terms-reference", "--attested-by"]);
...
function parse(argv) {
  ...
  if (command === "synthesize" && (!values["--provider"] || !values["--voice"])) invalid("provider-or-voice");
  return { command, projectDir: values["--project"], fixture: values["--fixture"], provider: values["--provider"], voiceId: values["--voice"], model: values["--model"], speakingRate: values["--speaking-rate"] || 1, termsReference: values["--terms-reference"], attestedBy: values["--attested-by"], commercialUseAttested: values["--commercial-use-attested"] === true, regenerate: values["--regenerate"] === true, dryRun: values["--dry-run"] === true, voiceCloned: values["--voice-cloned"] === true, impersonated: values["--impersonated"] === true };
}
const HELP = `Commands:\n  synthesize --fixture <approved.json> --project <dir> --provider <openai|mock> --voice <id> [--commercial-use-attested] [--regenerate] [--dry-run]\n  inspect --project <dir>\n  verify --fixture <approved.json> --project <dir>\n`;
