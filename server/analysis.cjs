...
const VALID_GOAL_ONLY_TIMING = Object.freeze({
  ...
  targetSegmentDuration: 35,
  referenceMaxSegmentDuration: 45,
  preContextSeconds: 12,
  maxPreContextSeconds: 24,
  postContextSeconds: 4.2,
  decisionPostSeconds: 1.4,
  minTransitionGapSeconds: 0.25,
  idealPreActionSeconds: 8,
  minPreActionSeconds: 4,
  maxShotToFinishSeconds: 28,
...
function inferredShotStartForGoalPhase({ sourceStart = 0, sourceEnd = null, finishTime = null } = {}) {
  ...
  const inferredLead = Math.min(
    ...
    Math.max(VALID_GOAL_ONLY_TIMING.minPreActionSeconds, availableLead * 0.22),
  );
  return Number(Math.max(start, Math.min(finish - 1, finish - inferredLead)).toFixed(2));
}
...
function normalizedGoalConfirmationTime({ confirmationTime = null, finishTime = null, sourceEnd = null } = {}) {
  const confirmation = finiteNumber(confirmationTime, null);
  if (confirmation === null) return null;
  const finish = finiteNumber(finishTime, null);
  if (finish === null || confirmation >= finish - 0.25) return confirmation;
  const end = finiteNumber(sourceEnd, null);
  if (end !== null && end >= finish + VALID_GOAL_ONLY_TIMING.minPostConfirmationSeconds) {
    return Number(Math.max(finish, end - VALID_GOAL_ONLY_TIMING.idealPostConfirmationSeconds).toFixed(2));
  }
  return finish;
}
...
function goalPhaseCoverageForEvent(event = {}, window = null) {
  ...
  const raw = event.phaseCoverage && typeof event.phaseCoverage === "object" && !Array.isArray(event.phaseCoverage)
    ? event.phaseCoverage
    : {};
  const finishFrameEvidence = raw.finishFrameEvidence ||
    (raw.visualGoalPayoff && raw.visualGoalPayoff.finishFrameEvidence) ||
    event.finishFrameEvidence ||
    null;
  const visualPayoffWithFinishFrame = finishFrameEvidence
  ...
  const earlyShotAnchor = explicitShotStart !== null &&
    confirmedGoalLike &&
    rawFinishTime - explicitShotStart > VALID_GOAL_ONLY_TIMING.maxShotToFinishSeconds;
  const shotStart = explicitShotStart == null || boundaryShotPlaceholder || earlyShotAnchor
  ...
  const rawConfirmationTime = finiteNumber(
  ...
  const confirmationTime = normalizedGoalConfirmationTime({
    confirmationTime: rawConfirmationTime,
    finishTime,
    sourceEnd: eventSourceEnd,
  });
  ...
  return {
    ...
    hasFinish: raw.hasFinish == null
      ? visualPayoffWithFinishFrame.hasVisibleGoalPayoff
      : Boolean(raw.hasFinish) && visualPayoffWithFinishFrame.hasVisibleGoalPayoff,
...
function compactGoalWindowForReferencePacing({
  sourceStart = 0,
  sourceEnd = 0,
  shotStart = null,
  finishTime = null,
  ...
  let start = finiteNumber(sourceStart, 0);
  let end = finiteNumber(sourceEnd, start + VALID_GOAL_ONLY_TIMING.minSegmentDuration);
  const mediaDuration = Math.max(0, Number(duration) || 0);
  if (mediaDuration > 0) end = Math.min(mediaDuration, end);

  const shot = finiteNumber(shotStart, null);
  const finish = finiteNumber(finishTime, null);
  const confirmation = finiteNumber(confirmationTime, finish);
  if (finish !== null) {
    end = Math.max(end, finish + VALID_GOAL_ONLY_TIMING.postContextSeconds);
  }
  if (confirmation !== null) {
    end = Math.max(end, confirmation + VALID_GOAL_ONLY_TIMING.decisionPostSeconds);
  }
  if (mediaDuration > 0) end = Math.min(mediaDuration, end);
  if (confirmation !== null) {
    const latestUsefulEnd = confirmation + VALID_GOAL_ONLY_TIMING.maxUsefulPostConfirmationSeconds;
    const requiredEnd = Math.max(
      finish === null ? start + VALID_GOAL_ONLY_TIMING.minSegmentDuration : finish + VALID_GOAL_ONLY_TIMING.postContextSeconds,
      confirmation + VALID_GOAL_ONLY_TIMING.decisionPostSeconds,
    );
    if (
      end > latestUsefulEnd &&
      latestUsefulEnd >= requiredEnd &&
      latestUsefulEnd - start >= VALID_GOAL_ONLY_TIMING.minSegmentDuration
    ) {
      end = latestUsefulEnd;
    }
  }

  const targetMax = VALID_GOAL_ONLY_TIMING.targetSegmentDuration;
  const referenceMax = VALID_GOAL_ONLY_TIMING.referenceMaxSegmentDuration;
  if (end - start <= targetMax) {
    return { sourceStart: Number(start.toFixed(2)), sourceEnd: Number(end.toFixed(2)) };
  }

  if (shot !== null) {
    const preferredStart = Math.max(start, shot - VALID_GOAL_ONLY_TIMING.idealPreActionSeconds);
    const targetStart = Math.max(start, end - targetMax);
    const latestStartWithMinPre = Math.max(0, shot - VALID_GOAL_ONLY_TIMING.minPreActionSeconds);
    start = Math.min(Math.max(preferredStart, targetStart), latestStartWithMinPre);
    if (end - start > referenceMax) {
      const hardStart = Math.max(start, end - referenceMax);
      start = Math.min(hardStart, latestStartWithMinPre);
    }
  } else {
    start = Math.max(start, end - targetMax);
  }

  if (end - start < VALID_GOAL_ONLY_TIMING.minSegmentDuration) {
    const missing = VALID_GOAL_ONLY_TIMING.minSegmentDuration - (end - start);
    start = Math.max(0, start - missing * 0.65);
    end = Math.min(mediaDuration || end + missing * 0.35, end + missing * 0.35);
  }

  return {
    sourceStart: Number(Math.max(0, start).toFixed(2)),
    sourceEnd: Number(Math.max(start + 3, end).toFixed(2)),
  };
}
...
function truthGoalWindowForPlan(event = {}, metadata = {}) {
  ...
  const compacted = compactGoalWindowForReferencePacing({
    sourceStart,
    sourceEnd,
    shotStart: phaseShotStart,
    finishTime,
    confirmationTime,
    duration,
  });
...
function truthGoalOutcomeForEvent(event = {}) {
  ...
  return normalizeGoalOutcome({
    ...
    decisionTimestamp: phaseCoverage.confirmationTime == null
...
function goalPhaseForCandidate(candidate = {}) {
  ...
  if (candidate.referenceGoalPacing && candidate.goalPhase) return candidate.goalPhase;
  return evidence.goalPhase ||
    candidate.goalPhase ||
...
function compilationHandlesForCandidate(candidate = {}) {
  ...
  if (isConfirmedGoalCandidate(candidate)) {
    if (moment.source === "match_event_truth_valid_goals_only") {
      return { pre: 0, post: 0 };
    }
    return {
      pre: Math.min(4, VALID_GOAL_ONLY_TIMING.idealPreActionSeconds),
      post: VALID_GOAL_ONLY_TIMING.minPostConfirmationSeconds,
    };
  }
...
function finishFrameTimeForCandidate(candidate = {}) {
  const phaseCoverage = goalPhaseCoverageForCandidate(candidate) || {};
  const phase = goalPhaseForCandidate(candidate) || {};
  const visualPayoff = phaseCoverage.visualGoalPayoff &&
    typeof phaseCoverage.visualGoalPayoff === "object"
    ? phaseCoverage.visualGoalPayoff
    : {};
  const phaseVisualPayoff = phase.visualGoalPayoff &&
    typeof phase.visualGoalPayoff === "object"
    ? phase.visualGoalPayoff
    : {};
  const evidence = candidate.finishFrameEvidence ||
    phaseCoverage.finishFrameEvidence ||
    visualPayoff.finishFrameEvidence ||
    phase.finishFrameEvidence ||
    phaseVisualPayoff.finishFrameEvidence ||
    (phase.phaseCoverage && phase.phaseCoverage.finishFrameEvidence) ||
    null;
...
function limitConfirmedGoalDurationWithHandles(candidate = {}, maxDuration = VALID_GOAL_ONLY_TIMING.maxSegmentDuration, mediaDuration = 0) {
  ...
  const compacted = compactGoalWindowForReferencePacing({
    ...
    shotStart: timing.shotStart,
    finishTime: timing.finishTime,
    confirmationTime: timing.confirmationTime,
    duration: mediaDuration,
  });
  if (
    compacted.sourceEnd - compacted.sourceStart >= VALID_GOAL_ONLY_TIMING.minSegmentDuration &&
    (
      compacted.sourceStart > currentStart + 0.05 ||
      compacted.sourceEnd < currentEnd - 0.05
    )
  ) {
    const next = {
      ...candidate,
...
function clampReferenceGoalCandidateDuration(candidate = {}, metadata = {}) {
  const mediaDuration = Math.max(0, Number(metadata.durationSeconds || 0));
  const timing = goalTimingForCandidate(candidate);
  const scoreChangeTime = scoreChangeTimeForCandidate(candidate);
  ...
  let sourceStart = Number(candidate.sourceStart || 0);
  let sourceEnd = Number(candidate.sourceEnd || sourceStart + REFERENCE_GOAL_COMPILATION.targetSegmentDuration);
  const requiredEnd = Math.max(
  ...
  if (sourceEnd - sourceStart < REFERENCE_GOAL_COMPILATION.minSegmentDuration) {
    sourceEnd = Math.min(
      mediaDuration || sourceEnd + REFERENCE_GOAL_COMPILATION.minSegmentDuration,
      sourceStart + REFERENCE_GOAL_COMPILATION.minSegmentDuration,
    );
  }
...
function compactConfirmedGoalCandidateForReference(candidate = {}, metadata = {}) {
  ...
  if (scoreChangeTime !== null) {
    const timing = goalTimingForCandidate(candidate);
    const shotStart = finiteNumber(timing.shotStart, null);
    const finishTime = finiteNumber(timing.finishTime, null);
    const finishFrameTime = finishFrameTimeForCandidate(candidate);
    const visibleFinishTime = Math.max(
    ...
    if (mediaDuration > 0) sourceEnd = Math.min(mediaDuration, sourceEnd);
    return clampReferenceGoalCandidateDuration(
      withReferenceGoalWindow(candidate, { sourceStart, sourceEnd }, metadata),
      metadata,
    );
  }
...
function trimReferenceGoalCompilationDuration(candidates = [], metadata = {}) {
  ...
  for (let index = sorted.length - 1; index >= 0 && totalDuration > REFERENCE_GOAL_COMPILATION.maxTotalDuration; index -= 1) {
    ...
    const finishTime = Math.max(
      finiteNumber(candidate.finishTime, 0),
      finiteNumber(finishFrameTimeForCandidate(candidate), 0),
    );
...
function abruptCutRisksForSegment(segment = {}) {
  ...
  if (confirmationTime !== null && sourceEnd - confirmationTime > VALID_GOAL_ONLY_TIMING.maxUsefulPostConfirmationSeconds) {
    risks.push("excessive_confirmation_tail");
  }
...
function referenceStyleQaForCompilation({ segments = [], captions = [], transitionPlan = [], totalDuration = 0, stylePreset = "social_sports_v1" } = {}) {
  ...
  const excessiveTailCount = safeSegments.filter((segment) => {
    const confirmationTime = finiteNumber(segment.confirmationTime, null);
    const sourceEnd = finiteNumber(segment.sourceEnd, null);
    return confirmationTime !== null &&
      sourceEnd !== null &&
      sourceEnd - confirmationTime > VALID_GOAL_ONLY_TIMING.maxUsefulPostConfirmationSeconds;
  }).length;
  const nonGoalFillerCount = safeSegments.filter((segment) => !(
    segment.highlightType === "goal" &&
    segment.goalOutcome &&
    segment.goalOutcome.outcome === "confirmed_goal"
  )).length;
  const excessiveTailRate = safeSegments.length ? Number((excessiveTailCount / safeSegments.length).toFixed(4)) : 0;
  const nonGoalFillerRate = safeSegments.length ? Number((nonGoalFillerCount / safeSegments.length).toFixed(4)) : 0;
  ...
  const score = Number((
    ...
    visibleGoalPayoffScore * 0.18 +
    durationScore * 0.11 +
    actionBoundaryScore * 0.16 +
    captionAlignment.captionAlignmentScore * 0.12 +
    (1 - nonGoalFillerRate) * 0.08
  ).toFixed(4));
  return {
    contractVersion: 1,
    ...
    referenceSimilarityNotes: [
      ...
      referencePacingScore >= 0.95 ? "reference_pacing_duration" : "goal_segments_need_tighter_pacing",
      nonGoalFillerRate === 0 ? "no_non_goal_filler" : "non_goal_filler_needs_review",
...
function selectCompilationCandidates(singleCandidates = [], metadata = {}) {
  ...
  if (confirmedGoalCandidates.length >= 2) {
    ...
    if (referenceGoalPacingApplies(distinctConfirmedGoalCandidates)) {
      ...
      const finalDuration = Number(chronological
        .reduce((sum, candidate) => sum + Number(candidate.sourceEnd - candidate.sourceStart), 0)
        .toFixed(2));
      if (
  ...
  const selectedConfirmedGoals = selected
    .filter(isConfirmedGoalCandidate)
    .sort((a, b) => a.sourceStart - b.sourceStart);
  if (selectedConfirmedGoals.length >= MULTI_MOMENT_COMPILATION.minSegments) {
    const confirmedChronological = expandCompilationCandidateWindows(selectedConfirmedGoals, metadata);
    const confirmedDuration = Number(confirmedChronological
      .reduce((sum, candidate) => sum + Number(candidate.sourceEnd - candidate.sourceStart), 0)
      .toFixed(2));
    if (
      confirmedDuration >= MULTI_MOMENT_COMPILATION.minTotalDuration &&
      confirmedDuration <= MULTI_MOMENT_COMPILATION.maxTotalDuration
    ) {
      return confirmedChronological.slice(0, MULTI_MOMENT_COMPILATION.maxSegments);
    }
  }
  const chronological = expandCompilationCandidateWindows(
    selected.sort((a, b) => a.sourceStart - b.sourceStart),
...
function createMultiMomentCompilationPlan({ singleCandidates, metadata, title, renderStylePreset, styleTarget, editIntensity, mediaSignals }) {
  ...
  const segments = selectedCandidates.map((candidate, index) => {
    ...
    const segment = {
      ...
      scoreBefore: safeScoreText(candidate.scoreBefore || (candidatePhase && candidatePhase.scoreBefore)),
      scoreAfter: safeScoreText(candidate.scoreAfter || (candidatePhase && candidatePhase.scoreAfter)),
      scoreChangeTime: scoreChangeTimeForCandidate(candidate),
      buildupStart,
      shotStart,
      finishTime,
      confirmationTime,
...
function createCandidateEditPlans({
  ...
  const allSingleCandidates = candidateMoments.map((moment) => {
    ...
    const momentGoalPhase = moment.evidence &&
      moment.evidence.goalPhase &&
      typeof moment.evidence.goalPhase === "object"
      ? moment.evidence.goalPhase
      : null;
    const momentPhaseCoverage = momentGoalPhase &&
      momentGoalPhase.phaseCoverage &&
      typeof momentGoalPhase.phaseCoverage === "object"
      ? momentGoalPhase.phaseCoverage
      : null;
    const momentFinishFrameEvidence = momentGoalPhase &&
      (
        momentGoalPhase.finishFrameEvidence ||
        (momentPhaseCoverage && momentPhaseCoverage.finishFrameEvidence) ||
        (
          momentPhaseCoverage &&
          momentPhaseCoverage.visualGoalPayoff &&
          momentPhaseCoverage.visualGoalPayoff.finishFrameEvidence
        )
      )
      ? (
          momentGoalPhase.finishFrameEvidence ||
          momentPhaseCoverage.finishFrameEvidence ||
          momentPhaseCoverage.visualGoalPayoff.finishFrameEvidence
        )
      : null;
    ...
    const plan = {
      ...
      highlightType,
      goalOutcome,
      goalPhase: momentGoalPhase,
      phaseCoverage: momentPhaseCoverage,
      scoreBefore: safeScoreText(momentGoalPhase && momentGoalPhase.scoreBefore),
      scoreAfter: safeScoreText(momentGoalPhase && momentGoalPhase.scoreAfter),
      scoreChangeTime: scoreChangeTimeForCandidate({
        goalPhase: momentGoalPhase,
        goalOutcome,
        analysisMoment: moment,
      }),
      shotStart: momentGoalPhase && Number.isFinite(Number(momentGoalPhase.shotStart))
        ? Number(Number(momentGoalPhase.shotStart).toFixed(2))
        : null,
      finishTime: momentGoalPhase && Number.isFinite(Number(momentGoalPhase.finishTime))
        ? Number(Number(momentGoalPhase.finishTime).toFixed(2))
        : null,
      confirmationTime: momentGoalPhase && Number.isFinite(Number(momentGoalPhase.confirmationTime))
        ? Number(Number(momentGoalPhase.confirmationTime).toFixed(2))
        : null,
      finishFrameEvidence: momentFinishFrameEvidence,
