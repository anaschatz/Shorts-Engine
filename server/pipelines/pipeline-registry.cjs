...
function normalizeNarratedJobPayload(payload = {}, action) {
  ...
  if (safeAction === "draft_narrated_short") {
    ...
    normalized.captionProfileVersion = sanitizeText(payload.captionProfileVersion || "1.1.0", 20).toLowerCase();
    ...
    if (hasAnimationBindings) {
      ...
      if (normalized.animationProvider !== "hyperframes_local" || normalized.animationRuntimeVersion !== "0.7.55" || normalized.animationStyleVersion !== "1.7.0") throw new AppError("VALIDATION_ERROR", SAFE_MESSAGES.VALIDATION_ERROR, 400, { field: "animationVersion" });
    }
