...
function synthesizeWithKokoro(request, options = {}) {
  ...
  const segments = request.pacingPlan ? request.pacingPlan.segments.map((segment) => ({ text: segment.text, speed: segment.speakingRate, pauseAfterMs: segment.pauseAfterMs })) : [{ text: request.script, speed: request.speakingRate, pauseAfterMs: 0 }];
  const expectedPauseMs = segments.reduce((sum, segment) => sum + segment.pauseAfterMs, 0);
  ...
  return new Promise((resolvePromise, rejectPromise) => {
    ...
    child.on("close", (code) => { if (settled) return; if (code !== 0 || !existsSync(outputPath)) return finish(new AppError("TTS_PROVIDER_FAILED", "Local Kokoro synthesis failed.", 502)); let result; try { result = JSON.parse(stdout); } catch { return finish(new AppError("TTS_PROVIDER_FAILED", "Local Kokoro returned an invalid result.", 502)); } if (result.status !== "complete" || result.segmentCount !== segments.length || result.totalPauseMs !== expectedPauseMs) return finish(new AppError("TTS_PROVIDER_FAILED", "Local Kokoro synthesis failed.", 502)); const buffer = readFileSync(outputPath); const requestIdentity = createHash("sha256").update(`${request.scriptHash}:${request.pacingHash || "unpaced"}`).digest("hex"); finish(null, { provider: "kokoro_local", model: request.model, voiceId: request.voiceId, audioFormat: "wav", buffer, providerRequestId: `local_${requestIdentity.slice(0, 24)}` }); });
    child.stdin.end(JSON.stringify({ segments, voice: request.voiceId, language: request.language, modelPath: runtime.modelPath, voicesPath: runtime.voicesPath, outputPath }));
  });
}
