...
const { normalizeSynthesisRequest, normalizeTtsProvenance, sha256 } = require("./contract.cjs");
...
async function synthesizeTtsNarration(input = {}, dependencies = {}) {
  ...
  const request = normalizeSynthesisRequest({ script: script.preparation.spokenText, language: script.draft.brief.language, provider: input.provider, model: input.model, voiceId: input.voiceId, speakingRate: input.speakingRate, voiceCloned: input.voiceCloned, impersonated: input.impersonated });
  const plan = { status: "dry_run", provider: request.provider, model: request.model, voiceId: request.voiceId, language: request.language, speakingRate: request.speakingRate, scriptSha256: request.scriptHash, approved: true, outputFiles: [AUDIO_FILE, MANIFEST_FILE], requiredEnvironmentVariables: request.provider === "openai" ? ["OPENAI_API_KEY"] : [], publishable: false };
  ...
  if ((existsSync(output.audio) || existsSync(output.manifest)) && !input.regenerate) {
    try {
      ...
      const same = manifest.provider === request.provider && manifest.model === request.model && manifest.voiceId === request.voiceId && manifest.language === request.language && manifest.speakingRate === request.speakingRate && manifest.script.sha256 === request.scriptHash;
  ...
  try {
    ...
    const manifest = normalizeTtsProvenance({ schemaVersion: "dark_curiosity_tts_provenance_v1", projectId, runId: `tts_${sha256(`${request.scriptHash}:${request.provider}:${request.model}:${request.voiceId}:${request.speakingRate}`).slice(0, 32)}`, script: { path: relative(REPO_ROOT, script.path) || basename(script.path), sha256: request.scriptHash, approvalReference: script.approvalReference, approved: true }, provider: result.provider, model: result.model, voiceId: result.voiceId, language: request.language, speakingRate: request.speakingRate, synthesizedAt, providerRequestId: result.providerRequestId || null, license: { termsReference: input.termsReference || "operator-attestation:provider-terms-reviewed", commercialUseAttested: input.commercialUseAttested === true, attestedBy: input.attestedBy || "local_operator" }, voiceCloned: false, impersonated: false, audio: { path: AUDIO_FILE, ...media }, dryRun: false });
...
module.exports = { AUDIO_FILE, MANIFEST_FILE, inspectTtsNarration, paths, scriptInfo, synthesizeTtsNarration, verifyTtsNarration };
