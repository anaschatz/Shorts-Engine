...
const VALUE_FLAGS = new Set(["--fixture", "--audio", "--operator-id", "--output-dir", "--render-profile", "--animation-profile", "--timeout-ms"]);
...
function parsePilotArgs(argv = []) {
  ...
  const animationProfile = values["--animation-profile"]
    ? String(values["--animation-profile"]).trim().toLowerCase()
    : null;
