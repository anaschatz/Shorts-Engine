...
function alignerDoctor(env = process.env, dependencies = {}) {
  ...
  const pythonAvailable = python.status === 0; const versionText = `${python.stdout || ""} ${python.stderr || ""}`.match(/Python\s+(\d+\.\d+\.\d+)/i); const pythonVersion = versionText ? versionText[1] : null; const versionParts = pythonVersion ? pythonVersion.split(".").map(Number) : []; const pythonSupported = versionParts[0] === 3 && versionParts[1] >= 9 && versionParts[1] < 14;
